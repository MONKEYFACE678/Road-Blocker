...
class Main_Frame(Tk):
    ...
    def about_us(self):
        ...
        Label(self.about, text=self.info, font=("Arial", 12), justify="left").pack(padx=20, pady=20)
    ...
    def data_window(self):
        ...
        def update_data(self):
            ...
            use_your_loc_button = Button(data_window, text="Use your current location", command=lg.get_location_coordinates_from_ip)
            use_your_loc_button.grid(row=0)
            
            e1 = Entry(data_window)
            e1.insert(0,"Enter Location Here...")
            e1.grid(row=1)
            
            use_entered_loc_button = Button(data_window, text="Submit address", command=lambda:lg.get_location_coordinates_from_address(e1.get()))
            use_entered_loc_button.grid(row=2)
            
            show_data_button =Button(data_window, text="Show data for location", command=update_data)
            show_data_button.grid(column=1)
            

            Start_data_window_button = Button(self, text="Get Data", command=data_window)
